...
func verifyOCIAttestation(_ context.Context, verifier signature.Verifier, att payloader) error {
	...
	if env.PayloadType != types.IntotoPayloadType {
		return cosignErrors.NewVerificationError(1, "invalid payloadType %s on envelope. Expected %s", env.PayloadType, types.IntotoPayloadType)
	}
...
func verifySignatures(ctx context.Context, sigs oci.Signatures, h v1.Hash, co *CheckOpts) (checkedSignatures []oci.Signature, bundleVerified bool, err error) {
	...
	if len(checkedSignatures) == 0 {
		return nil, false, cosignErrors.NewVerificationError(&cosignErrors.ErrNoMatchingSignatures{}, "no matching signatures: \n%s", strings.Join(validationErrs, "\n "))
		// return nil, false, cosignErrors.NewVerificationError(cosignErrors.ImageWithoutSignature, "%w:\n%s", "no matching attestations", strings.Join(validationErrs, "\n "))
...
func CheckExpiry(cert *x509.Certificate, it time.Time) error {
	...
	if cert.NotAfter.Before(it) {
		return cosignErrors.NewVerificationError(1, "certificate expired before signatures were entered in log: %s is before %s",
			ft(cert.NotAfter), ft(it))
	}
	if cert.NotBefore.After(it) {
		return cosignErrors.NewVerificationError(1, "certificate was issued after signatures were entered in log: %s is after %s",
			ft(cert.NotAfter), ft(it))
	}
	return nil
}
