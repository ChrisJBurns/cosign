...
func verifyOCIAttestation(ctx context.Context, verifier signature.Verifier, att payloader) error {
	...
	if env.PayloadType != types.IntotoPayloadType {
		return NewVerificationErrorJustMessage(fmt.Sprintf("invalid payloadType %s on envelope. Expected %s", env.PayloadType, types.IntotoPayloadType))
	}
...
func ValidateAndUnpackCert(cert *x509.Certificate, co *CheckOpts) (signature.Verifier, error) {
	...
	if !contains && len(co.SCT) == 0 {
		...
		return nil, NewVerificationErrorJustMessage("certificate does not include required embedded SCT and no detached SCT was set")
	}
...
func CheckCertificatePolicy(cert *x509.Certificate, co *CheckOpts) error {
	...
	if len(co.Identities) > 0 {
		...
		return NewVerificationErrorJustMessage(fmt.Sprintf("none of the expected identities matched what was in the certificate, got subjects [%s] with issuer %s",
...
func validateCertExtensions(ce CertExtensions, co *CheckOpts) error {
	if co.CertGithubWorkflowTrigger != "" {
		if ce.GetCertExtensionGithubWorkflowTrigger() != co.CertGithubWorkflowTrigger {
			// TODO: add error type instead of blank string
			return NewVerificationErrorJustMessage("expected GitHub Workflow Trigger not found in certificate")
		}
	}
	...
	if co.CertGithubWorkflowSha != "" {
		if ce.GetExtensionGithubWorkflowSha() != co.CertGithubWorkflowSha {
			// TODO: add error type instead of blank string
			return NewVerificationErrorJustMessage("expected GitHub Workflow SHA not found in certificate")
		}
	}
	...
	if co.CertGithubWorkflowName != "" {
		if ce.GetCertExtensionGithubWorkflowName() != co.CertGithubWorkflowName {
			// TODO: add error type instead of blank string
			return NewVerificationErrorJustMessage("expected GitHub Workflow Name not found in certificate")
		}
	}
	...
	if co.CertGithubWorkflowRepository != "" {
		if ce.GetCertExtensionGithubWorkflowRepository() != co.CertGithubWorkflowRepository {
			// TODO: add error type instead of blank string
			return NewVerificationErrorJustMessage("expected GitHub Workflow Repository not found in certificate")
		}
	}
	...
	if co.CertGithubWorkflowRef != "" {
		if ce.GetCertExtensionGithubWorkflowRef() != co.CertGithubWorkflowRef {
			// TODO: add error type instead of blank string
			return NewVerificationErrorJustMessage("expected GitHub Workflow Ref not found in certificate")
		}
	}
	return nil
}
	...
	if err != nil {
		if strings.Contains(err.Error(), "MANIFEST_UNKNOWN") {
			return nil, false, NewVerificationErrorWrapped(
				ErrImageTagNotFound,
				fmt.Sprintf("%v: %v", ErrImageTagNotFound.Error(), err.Error()),
			)
		}
...
func verifyInternal(ctx context.Context, sig oci.Signature, h v1.Hash,
	...
	if verifier == nil {
		...
		if cert == nil {
			// TODO: add error type instead of blank string
			return false, NewVerificationErrorJustMessage("no certificate found on signature")
		}
		...
		if !expirationChecked {
			if err := CheckExpiry(cert, time.Now()); err != nil {
				...
				if co.IgnoreTlog && acceptableRFC3161Time == nil {
					// TODO: add error type instead of blank string
					return false, NewVerificationErrorJustMessage("expected a signed timestamp to verify an expired certificate")
				}
				return false, fmt.Errorf("checking expiry on certificate with bundle: %w", err)
			}
		}
...
func CheckExpiry(cert *x509.Certificate, it time.Time) error {
	...
	if cert.NotAfter.Before(it) {
		return NewVerificationErrorJustMessage(fmt.Sprintf("certificate expired before signatures were entered in log: %s is before %s",
			ft(cert.NotAfter), ft(it)))
	}
	if cert.NotBefore.After(it) {
		return NewVerificationErrorJustMessage(fmt.Sprintf("certificate was issued after signatures were entered in log: %s is after %s",
			ft(cert.NotAfter), ft(it)))
	}
	return nil
}
...
func VerifyBundle(sig oci.Signature, co *CheckOpts) (bool, error) {
	...
	if !ok {
		// TODO: add error type instead of blank string
		return false, NewVerificationErrorJustMessage("verifying bundle: rekor log public key not found for payload")
	}
...
func compareSigs(bundleBody string, sig oci.Signature) error {
	...
	if bundleSignature != actualSig {
		// TODO: add error type instead of blank string
		return NewVerificationErrorJustMessage("signature in bundle does not match signature being verified")
	}
	return nil
}
...
func VerifySET(bundlePayload cbundle.RekorPayload, signature []byte, pub *ecdsa.PublicKey) error {
	...
	if !ecdsa.VerifyASN1(pub, hash[:], signature) {
		// TODO: add error type instead of blank string
		return NewVerificationErrorJustMessage("unable to verify SET")
	}
	return nil
}
